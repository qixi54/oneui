...
const databaseEnterpriseThemeMode = ref<"neutral" | "ops-console">("ops-console");
const databaseEnterpriseFailureMode = ref(false);
const databaseEnterpriseActionLog = ref<string[]>([
  "ThemeScope + DatabaseView middleware demo ready",
  "Edit any cell to trigger optimistic apply / revert",
]);
const databaseEnterpriseRecords = ref<DataRecord[]>(
  databaseDemoRecords.map((record) => ({
    ...record,
    fields: { ...record.fields },
  })),
);
const databaseEnterpriseSnapshots = new Map<string, CellValue>();

function databaseEnterprisePushLog(message: string) {
  databaseEnterpriseActionLog.value = [message, ...databaseEnterpriseActionLog.value].slice(0, 8);
}

function databaseEnterpriseRecordKey(rowId: string, fieldId: string) {
  return `${rowId}:${fieldId}`;
}

function toggleDatabaseEnterpriseThemeMode() {
  databaseEnterpriseThemeMode.value =
    databaseEnterpriseThemeMode.value === "ops-console" ? "neutral" : "ops-console";
}

function toggleDatabaseEnterpriseFailureMode() {
  databaseEnterpriseFailureMode.value = !databaseEnterpriseFailureMode.value;
}

function resetDatabaseEnterpriseLog() {
  databaseEnterpriseActionLog.value = [
    "manual log reset",
    "ThemeScope + DatabaseView middleware demo ready",
    "Edit any cell to trigger optimistic apply / revert",
  ];
  databaseEnterpriseFailureMode.value = false;
}

const databaseEnterpriseMiddleware = composeDatabaseViewMiddlewares(
  createDatabaseViewToastMiddleware({
    onSuccess: (message, context) => {
      if (context.action === "cell-edit") {
        databaseEnterprisePushLog(`toast: ${message}`);
      }
    },
  ...
  createDatabaseViewAnalyticsMiddleware({
    onEvent: (event) => {
      if (event.action !== "cell-edit") return;
      databaseEnterprisePushLog(`analytics: ${event.phase} ${event.action}`);
    },
  }),
  createDatabaseViewOptimisticMiddleware({
    apply: (context) => {
      ...
      const record = databaseEnterpriseRecords.value.find((item) => item.id === payload.rowId);
      if (!record) return;

      const key = databaseEnterpriseRecordKey(payload.rowId, payload.fieldId);
      databaseEnterpriseSnapshots.set(key, record.fields[payload.fieldId]);
      databaseEnterpriseRecords.value = databaseEnterpriseRecords.value.map((item) =>
      ...
      databaseEnterprisePushLog(`optimistic apply: ${payload.rowId}.${payload.fieldId}`);
    },
    revert: (context) => {
      if (context.action !== "cell-edit") return;
      const payload = context.payload as { rowId: string; fieldId: string };
      const key = databaseEnterpriseRecordKey(payload.rowId, payload.fieldId);
      const previousValue = databaseEnterpriseSnapshots.get(key);
      ...
      databaseEnterpriseSnapshots.delete(key);
      databaseEnterprisePushLog(`optimistic revert: ${payload.rowId}.${payload.fieldId}`);
    },
  }),
);

const databaseEnterpriseActions = {
  middleware: databaseEnterpriseMiddleware,
  onCellEdit: async (payload: { rowId: string; fieldId: string; value: unknown }) => {
    databaseEnterprisePushLog(`action onCellEdit: ${payload.rowId}.${payload.fieldId}`);
    if (databaseEnterpriseFailureMode.value) {
      databaseEnterpriseFailureMode.value = false;
...
const databaseEnterpriseSnippet = computed(() => `<ThemeScope theme="${databaseEnterpriseThemeMode.value}" tag="section" class="database-enterprise__scope">
  <DatabaseView
    table-id="enterprise-db"
    mode="local"
    :schema="databaseDemoSchema"
    :records="databaseEnterpriseRecords"
    :views="databaseDemoViewPresets"
    current-view-id="database-table"
    detail-presentation="side-panel"
    :actions="databaseEnterpriseActions"
    :auto-load="false"
  />
</ThemeScope>`);
            ...
            <div class="database-enterprise__card">
              <div class="database-enterprise__hero">
                ...
                <div class="database-enterprise__chips">
                  <span class="database-enterprise__chip">ThemeScope: {{ databaseEnterpriseThemeMode }}</span>
                  <span class="database-enterprise__chip">Middleware: toast / analytics / optimistic</span>
                  <span class="database-enterprise__chip">
                    Failure mode: {{ databaseEnterpriseFailureMode ? "armed" : "ready" }}
                  </span>
                </div>
              </div>

              <ThemeScope :theme="databaseEnterpriseThemeMode" tag="section" class="database-enterprise__scope">
                  ...
                  <aside class="database-enterprise__aside">
                    <div class="database-enterprise__aside-card">
                      ...
                      <div class="database-enterprise__control-row">
                        <button
                          class="dev-btn"
                          :class="{ 'dev-btn--info': databaseEnterpriseThemeMode === 'ops-console' }"
                          @click="toggleDatabaseEnterpriseThemeMode"
                        >
                          toggle ThemeScope
                        </button>
                        <button
                          class="dev-btn"
                          :class="{ 'dev-btn--warning': databaseEnterpriseFailureMode }"
                          @click="toggleDatabaseEnterpriseFailureMode"
                        >
                          {{ databaseEnterpriseFailureMode ? 'next edit will fail' : 'arm failure mode' }}
                        </button>
                        <button
                          class="dev-btn"
                          @click="resetDatabaseEnterpriseLog"
                        >
                          reset log
                        </button>
                      </div>
                    ...
                    <div class="database-enterprise__aside-card">
                      ...
                      <div class="database-enterprise__log">
                        <div
                          v-for="(entry, index) in databaseEnterpriseActionLog"
              ...
              <pre class="database-shell__code"><code>{{ databaseEnterpriseSnippet }}</code></pre>
            </div>
          </div>
